...
function buildExplanation(c: DiscoveryCandidate, signals: Signal[]): string {
  ...
  for (const s of signals) {
    ...
    if (s.id === "asking" && ev) parts.push(ev.text);
    else if (s.id === "maintainer" && ev) parts.push(ev.text);
    else if (s.id === "contributor" && ev) parts.push(ev.text);
  }
  const weak = signals.filter((s) => ["tech", "recency", "audience"].includes(s.id) && s.points > 0);
  if (weak.length) {
    parts.push(
      `Supporting context: ${weak.map((w) => `${w.label.toLowerCase()} +${w.points}`).join(" · ")}.`,
    );
  }
  if (!parts.length) parts.push("Only weak signals found — treat as a cold lead.");
  return parts.join(" ");
}
