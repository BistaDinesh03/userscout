...
interface Workspace {
  ...
  enrichProspectContacts: (prospectId: string) => Promise<{ channels: unknown[]; errors: string[]; fetched: string[] }>;
  loadContactChannels: (prospectId: string) => Promise<unknown[]>;
}
...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  const reload = useCallback(async () => {
    // Step 1: check session. Only clear the user if THIS fails.
    let userData: SafeUser | null = null;
    try {
      const meRes = await api.me() as any;
      const u = meRes.user || meRes;
      userData = { id: u.id, username: u.username, createdAt: new Date(u.created_at || Date.now()).getTime() };
      setUser(userData);
    } catch {
      // No valid session — that's fine, we're logged out.
      setUser(null);
      setProjects([]);
      setProspects([]);
      setEvents([]);
    ...
    // Step 2: load data. If this fails, keep the user logged in.
    try {
  ...
  const api_ = useMemo<Workspace>(() => {
    return {
      ...
      setStatus: async (prospectId, to, opts) => {
        await api.updateProspectStatus(prospectId, to, opts?.channel);
        await reload();
      },
      addNote: async (prospectId, text) => {
        await api.addOutreachEvent(prospectId, "note", text);
        await reload();
      },
      saveDraft: async (prospectId, channel, body) => {
        await api.addOutreachEvent(prospectId, "draft", body, channel);
        await reload();
      },
      saveFeedback: async (prospectId, input) => {
        await api.addFeedback(prospectId, input);
        await reload();
      },
      enrichProspectContacts: async (prospectId) => {
        const r = await api.enrichProspectContacts(prospectId) as { channels: unknown[]; errors: string[]; fetched: string[] };
        return r;
      },
      loadContactChannels: async (prospectId) => {
        const r = await api.listContactChannels(prospectId) as { channels: unknown[] };
        return r.channels || [];
      },
    };
  }, [ready, user, token, projects, prospects, events, drafts, feedback, community, rate, toasts, gh, toast, dismissToast, reload]);
